...
def _test(args):
    ...
    model._network.load_state_dict(torch.load('./logs/cifar100/10_10_sip/InfLoRA/adam/10/0.95_1.0-0.0005/0/task_9.pth'))
    model._network.eval()  # 设置模型为评估模式

    cnn_curve, cnn_curve_with_task, nme_curve, cnn_curve_task = {'top1': []}, {'top1': []}, {'top1': []}, {'top1': []}

    time_start = time.time()
    cnn_accy, cnn_accy_with_task, nme_accy, cnn_accy_task = model.test(data_manager.nb_tasks)
