    ...
    def train_function(self, train_loader, test_loader, optimizer, scheduler):
        ...
        for _, epoch in enumerate(prog_bar):
            ...
            for i, (_, inputs, targets) in enumerate(train_loader):
                ...
                logits = self._network(inputs, get_x_feat=True)['logits']
                ...
                if self._cur_task > 0:
                    ...
                    for module in self._network.modules():
                        if isinstance(module, Attention_LoRA):
                            if kk > 0:
                                x_matrix = module.x_matrix
                                # print(x_matrix.max())
                                # print(self.feature_mat[kk])
                                if self.project_type[kk] == 'remove':
                                    x_matrix_projection = x_matrix - torch.mm(self.feature_mat[kk].to(x_matrix.device),x_matrix)
                                else:
                                    assert self.project_type[kk] == 'retain'
                                    x_matrix_projection = torch.mm(self.feature_mat[kk].to(x_matrix.device),x_matrix)
                                loss_new += F.mse_loss(x_matrix, x_matrix_projection)
                            kk += 1
                    # print(loss)
                    # print(loss_new)
                    # exit()
